...
for n in range(np.shape(F)[1]):
    ax.plot([0, F[0, n]], [0, F[1, n]],
            label=total_genres[n], color=plt.cm.tab20(n))

ax.tick_params(axis='both', which='major', labelsize=14)
leg_ax = fig.add_subplot(gs[0])
leg_ax.axis('off')
leg_ax.legend(handles=ax.lines, loc='center',
              bbox_to_anchor=(0.5, 0.5), frameon=False, fontsize=14)
